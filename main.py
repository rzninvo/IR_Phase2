...
if __name__ == '__main__':
    ...
    query = input()
    if option == '1':
        query_result = IR().cosine_similarity_search(len(content), weighted_posting_list, champion_list, query)
        IR().print_cosine_search_result(query_result, titles)
    else:
        query_result = IR().cosine_similarity_search(len(content), weighted_posting_list, champion_list, query, speedup= 1)
        IR().print_cosine_search_result(query_result, titles)
